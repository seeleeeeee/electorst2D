...
// ============ FEM РЕШАТЕЛЬ (РЕАЛЬНОЕ РЕШЕНИЕ УРАВНЕНИЯ ПУАССОНА) ============
class FemSolver {
public:
                      ...
                      electrostatic::Solution& solution)
    {
        ...
        const double chargeDensity = 1e-9;
        ...
        for (const auto& elem : mesh->elements) {
            ...
            // Правая часть: вклад от объемной плотности заряда
            double rho_term = chargeDensity * area / 3.0;
            b(n0) += rho_term;
            b(n1) += rho_term;
            b(n2) += rho_term;
        }
        ...
        std::cout << "[Solver] Matrix assembled, nnz=" << K.nonZeros() << std::endl;
        
        // Применение граничных условий (методом штрафа)
    ...
    , currentMaterialName("Air")
{
    ...
    // Панель материалов с возможностью выбора
...
void MainWindow::on_actionAnalyze_triggered()
{
    ...
    // Сбор граничных условий
    std::vector<FemSolver::BC> bcs;
    double minX = 1e9, maxX = -1e9, minY = 1e9, maxY = -1e9;
    for (auto& n : currentMesh->nodes) {
        minX = qMin(minX, n.x); maxX = qMax(maxX, n.x);
        minY = qMin(minY, n.y); maxY = qMax(maxY, n.y);
    }
    
    for (int row = 0; row < ui->tableBoundary->rowCount(); row++) {
        QTableWidgetItem* nameItem = ui->tableBoundary->item(row, 0);
        QTableWidgetItem* valItem = ui->tableBoundary->item(row, 1);
        if (!nameItem || !valItem) continue;
        
        QString name = nameItem->text();
        double voltage = valItem->text().toDouble();
        
        for (size_t i = 0; i < currentMesh->nodes.size(); i++) {
            const auto& n = currentMesh->nodes[i];
            bool apply = false;
            if (name == "Left" && fabs(n.x - minX) < 1) apply = true;
            else if (name == "Right" && fabs(n.x - maxX) < 1) apply = true;
            else if (name == "Bottom" && fabs(n.y - minY) < 1) apply = true;
            else if (name == "Top" && fabs(n.y - maxY) < 1) apply = true;
            
            if (apply) bcs.push_back({(int)i, voltage});
        }
    }
    
    // Запуск FEM решателя
    if (!FemSolver::solve(currentMesh, bcs, currentEpsilon, currentSolution)) {
        updateStatusBar("Solver failed!");
        QMessageBox::warning(this, "Error", "FEM solver failed to converge!");
        return;
    }
    
    // Поиск максимального потенциала для отображения
    ...
    updateStatusBar(QString("Solved! Material: %1 (ε=%2), Energy: %3 J, Potential range: [%4, %5] V")
        .arg(currentMaterialName)
        .arg(currentEpsilon)
        .arg(currentSolution.energy, 0, 'e', 3)
