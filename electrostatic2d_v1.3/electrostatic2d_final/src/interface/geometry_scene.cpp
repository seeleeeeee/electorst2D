...
void GeometryScene::setMode(Mode mode)
{
    currentMode = mode;
    if (mode == AddNodeMode)
        emit statusMessage("Режим: ТОЧКИ (клик левой кнопкой)");
    else if (mode == AddEdgeMode)
        emit statusMessage("Режим: ЛИНИИ (зажми на точке и тяни)");
    else
        emit statusMessage("Режим: ВЫДЕЛЕНИЕ");
}
...
void GeometryScene::clearAll()
{
    ...
    drawing = false;
    isDragging = false;
    emit statusMessage("Все очищено");
}
...
{
    QGraphicsEllipseItem* node = addEllipse(-8, -8, 16, 16, QPen(Qt::black, 2), QBrush(Qt::white));
    node->setPos(pos);
...
void GeometryScene::updateHoverNode(const QPointF& pos)
{
    ...
    if (foundNode != hoverNode) {
        ...
        hoverNode = foundNode;
        if (hoverNode) {
            hoverNode->setBrush(QBrush(Qt::yellow));
            hoverNode->setPen(QPen(Qt::red, 2));
        }
    }
}
...
void GeometryScene::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton) {
        ...
        if (currentMode == AddNodeMode) {
            ...
            nodes.append(node);
            emit statusMessage(QString("Точка: (%1, %2)").arg(pos.x()).arg(pos.y()));
            qDebug() << "  точка создана, всего:" << nodes.size();
        }
        else if (currentMode == AddEdgeMode) {
            startNode = nullptr;
            for (auto node : nodes) {
                double dist = QLineF(node->pos(), pos).length();
                qDebug() << "  проверяю точку" << node->pos() << "расстояние:" << dist;
                if (dist < 50) {
                    startNode = node;
                    qDebug() << "  >>> НАЙДЕНА!";
                    break;
            ...
            if (startNode) {
                ...
                emit statusMessage("Тяни к другой точке");
                qDebug() << "  начата линия от:" << startNode->pos();
            } else {
                emit statusMessage("Зажми на точке! (кликни на белую точку)");
                qDebug() << "  точка не найдена, всего точек:" << nodes.size();
            }
        }
    }
...
void GeometryScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (event->button() == Qt::LeftButton && drawing && isDragging) {
        ...
        if (endNode) {
            ...
            emit statusMessage("ЛИНИЯ ДОБАВЛЕНА!");
            qDebug() << "  линия создана!";
        } else {
            emit statusMessage("Отпусти на другой точке! (кликни на точку)");
            qDebug() << "  конечная точка не найдена";
        }
